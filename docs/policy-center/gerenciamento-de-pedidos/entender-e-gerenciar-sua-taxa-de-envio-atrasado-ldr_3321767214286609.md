...
* Os vendedores devem manter uma **LDR de 4% ou inferior**.
* Os pedidos devem ser enviados dentro de **2 dias úteis** para não serem contados como "atrasados".
* O desempenho da LDR é visível na sua página **Integridade da loja**.

# O que é LDR?

**A Taxa de envio atrasado (LDR)** mede a porcentagem de pedidos no período de cálculo aplicável que não mudaram para o status "Trânsito para envio" (ou TTS), Enviado ou Despachado dentro de dois dias úteis. Essa métrica inclui tanto os pedidos despachados com atraso como os pedidos que continuam não enviados após o prazo de envio.***Observação:*** Taxa de envio atrasado (LDR) = (Pedidos enviados com atraso + Pedidos já atrasados não enviados) ÷ (Total de pedidos com envio previsto para esse período).

* Atrasado = não despachado em até 2 dias úteis
* Pedidos cancelados antes do SLA de Trânsito para envio são excluídos do cálculo da LDR.
* Pedidos de amostra grátis são excluídos do cálculo da LDR.
...
|  |  |  |
| --- | --- | --- |
| **Método de envio** | **SLA: Envio/TikTok Shop** | **Cancelamento automático** |
| Envio via TikTok/vendedor | Dentro de 2 dias úteis | Cancela automaticamente após 6 dias úteis se não for enviado |

* **Horário de corte:** às 23:59:59 (hora local) no dia do SLA.
* Os **dias úteis** *excluem* finais de semana e feriados públicos (por exemplo: 1º de janeiro, 1º de maio; consulte [aqui](https://seller-br.tiktok.com/university/essay?identity=1&role=1&knowledge_id=1444948508280592&from=policy "https://seller-br.tiktok.com/university/essay?identity=1&role=1&knowledge_id=1444948508280592&from=policy") para ver a lista completa).
...
3. Clique em **Ver detalhes** > **Ver pedidos com defeitos** e revise a análise da LDR no nível do pedido.
   ...
   ![image](https://p16-oec-university-sign-sg.ibyteimg.com/tos-alisg-i-nk3i2mqmvs-sg/dd5c1ed907e44447a3f85ae16818b153~tplv-nk3i2mqmvs-image.png?lk3s=5d1a069b&x-expires=2073550187&x-signature=eExjWLN5V1uC6CslvDaVtdfPcsQ%3D)
